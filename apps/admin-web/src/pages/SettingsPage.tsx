...
export function SettingsPage() {
  ...
  const updateForm = (patch: Partial<V2StoreConfigSaveInput>) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const next = { expectedVersion: config.version ?? 1, storeName: config.storeName, announcement: config.announcement, businessOpen: config.businessOpen, dayBoundaryTime: config.dayBoundaryTime };
      writeResourceCache(SETTINGS_CACHE_KEY, next);
      if (editRevision.current === startedAtRevision) setForm(next);
      setError("");
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const nextForm = { expectedVersion: next.version ?? form.expectedVersion + 1, storeName: next.storeName, announcement: next.announcement, businessOpen: next.businessOpen, dayBoundaryTime: next.dayBoundaryTime };
      writeResourceCache(SETTINGS_CACHE_KEY, nextForm);
    ...
    <div className="page-stack settings-page">
      ...
      <form className="settings-grid" onSubmit={save}>
        <section className="panel settings-section">
          ...
          <label className="field"><span>摊位名称</span><input value={form.storeName} onChange={(event) => updateForm({ storeName: event.target.value })} /></label>
          <label className="field"><span>今日公告</span><textarea rows={3} value={form.announcement ?? ""} onChange={(event) => updateForm({ announcement: event.target.value })} placeholder="例如：每日现打，售完即止" /></label>
        </section>
        <section className="panel settings-section">
          ...
          <label className={`business-toggle ${form.businessOpen ? "is-open" : ""}`}>
            <input type="checkbox" checked={form.businessOpen} onChange={(event) => updateForm({ businessOpen: event.target.checked })} />
        ...
        <section className="panel settings-section">
          ...
          <label className="field"><span>切换时间</span><input type="time" value={form.dayBoundaryTime} onChange={(event) => updateForm({ dayBoundaryTime: event.target.value })} /></label>
