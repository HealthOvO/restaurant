...
// src/handlers/order.create.ts
var order_create_exports = {};
__export(order_create_exports, {
  main: () => main
});
module.exports = __toCommonJS(order_create_exports);
...
function createMemberCode(memberId) {
...
function assertOrderSubmissionReady(input) {
...
function buildLineId(menuItemId, selectedOptions) {
  const optionKey = selectedOptions.map((item) => `${item.groupId}:${item.choiceId}`).sort().join("|");
  return optionKey ? `${menuItemId}__${optionKey}` : menuItemId;
}
function sortOptions(groups, selectedOptions) {
  const groupIndex = new Map(groups.map((group, index) => [group._id, index]));
  return selectedOptions.slice().sort((left, right) => {
...
function applyDefaultChoices(group, selectedChoices) {
  if (selectedChoices.length > 0) {
    return selectedChoices;
  }
  return group.choices.filter((choice) => choice.isEnabled && choice.isDefault);
}
function resolveSelectedOptions(menuItem, requestedOptions) {
  const groups = menuItem.optionGroups ?? [];
  if (groups.length === 0) {
    return [];
  }
  const selectedOptionsByGroup = requestedOptions.reduce((result, option) => {
...
function buildRequestScopedId(prefix, requestId) {
...
function buildOrderNo(orderId, now) {
...
function createDefaultStoreConfig(storeId, now) {
  return {
...
function createDefaultCategories(storeId, now) {
  return [
    {
...
function createDefaultMenuItems(storeId, now) {
  return [
    {
      _id: "dish-fat-beef",
      storeId,
...
async function ensureOrderingSeeds(repository) {
...
async function ensureMemberShell(repository, openId) {
...
// src/handlers/order.create.ts
var main = defineHandler(async ({ context, event, repository }) => {
