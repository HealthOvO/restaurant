...
// src/handlers/staff.feedback.submit.ts
var staff_feedback_submit_exports = {};
__export(staff_feedback_submit_exports, {
  main: () => main
});
module.exports = __toCommonJS(staff_feedback_submit_exports);
...
function createFeedbackCode(feedbackId) {
...
// src/runtime/service.feedback.ts
function nowIso() {
...
async function submitStaffFeedback(repository, input) {
  ...
  const ticket = {
    ...
    contactName: normalizeOptionalText2(parsed.contactName) ?? staff.displayName,
    contactInfo: normalizeOptionalText2(parsed.contactInfo) ?? staff.username,
    sourcePage: normalizeOptionalText2(parsed.sourcePage),
    createdAt: now,
    updatedAt: now
  };
...
// src/handlers/staff.feedback.submit.ts
var main = defineHandler(async ({ event, repository }) => submitStaffFeedback(repository, event));
