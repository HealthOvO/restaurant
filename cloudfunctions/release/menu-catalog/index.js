...
// src/handlers/menu.catalog.ts
var menu_catalog_exports = {};
__export(menu_catalog_exports, {
  main: () => main
});
module.exports = __toCommonJS(menu_catalog_exports);
...
function buildMenuCatalog(categories, items) {
  ...
  const visibleItems = sortByOrder(items).filter((item) => item.isEnabled).filter((item) => visibleCategoryIds.has(item.categoryId));
  return {
...
async function getMenuCatalog(repository, input = {}) {
...
// src/handlers/menu.catalog.ts
var main = defineHandler(async ({ event, repository }) => getMenuCatalog(repository, event));
