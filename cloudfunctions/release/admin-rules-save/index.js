...
// src/handlers/admin.rules.save.ts
var admin_rules_save_exports = {};
__export(admin_rules_save_exports, {
  main: () => main
});
module.exports = __toCommonJS(admin_rules_save_exports);
...
function assertPointExchangeItemsValid(items) {
  items.forEach((item) => {
    if (!item.name.trim()) {
...
function buildRewardRuleSaveSummary(rules, exchangeItems = []) {
...
function normalizeVoucherTemplate(voucherTemplate) {
  if (!voucherTemplate) {
    return void 0;
  }
  return {
...
async function saveRules(repository, input) {
  ...
  const normalizedInputRules = parsed.rules.slice().sort((left, right) => {
  ...
  const rules = normalizedInputRules.map((rule, index) => ({
    _id: rule._id ?? createId("rule"),
    storeId: repository.storeId,
    name: rule.name.trim(),
    type: rule.type,
    threshold: rule.type === "INVITE_MILESTONE" ? rule.threshold : void 0,
    rewardMode: rule.type === "INVITE_MILESTONE" ? rule.rewardMode ?? "ONCE" : void 0,
  ...
  const normalizedExchangeItems = parsed.exchangeItems.slice().sort((left, right) => {
    if (left.sortOrder !== right.sortOrder) {
      return left.sortOrder - right.sortOrder;
    }
    return (left._id ?? "").localeCompare(right._id ?? "");
  });
  const exchangeItems = normalizedExchangeItems.map((item, index) => ({
    ...
    voucherTemplate: normalizeVoucherTemplate(item.voucherTemplate),
...
// src/handlers/admin.rules.save.ts
var main = defineHandler(async ({ event, repository }) => saveRules(repository, event));
