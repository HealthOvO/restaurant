...
// src/handlers/admin.menu.save.ts
var admin_menu_save_exports = {};
__export(admin_menu_save_exports, {
  main: () => main
});
module.exports = __toCommonJS(admin_menu_save_exports);
...
// ../packages/shared/src/domain/menu.ts
function assertMenuConfigValid(categories, items) {
  const enabledCategoryIds = new Set(
    categories.filter((category) => category.isEnabled).map((category) => category._id)
  );
  if (categories.length === 0) {
    throw new DomainError("MENU_CATEGORY_REQUIRED", "\u81F3\u5C11\u9700\u8981\u914D\u7F6E\u4E00\u4E2A\u83DC\u54C1\u5206\u7C7B");
  }
  const seenCategoryNames = /* @__PURE__ */ new Set();
...
// src/handlers/admin.menu.save.ts
var main = defineHandler(async ({ event, repository }) => saveAdminMenu(repository, event));
