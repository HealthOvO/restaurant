...
// src/handlers/admin.binding.adjust.ts
var admin_binding_adjust_exports = {};
__export(admin_binding_adjust_exports, {
  main: () => main
});
module.exports = __toCommonJS(admin_binding_adjust_exports);
...
function assertInviteAdjustmentAllowed(params) {
  ...
  if (existingRelation && existingRelation.inviterMemberId === inviterMemberId && existingRelation.inviteeMemberId === inviteeMemberId) {
...
function resolveInviteRewardUnitPoints(rule, pointTransactions) {
  const historicalTransaction = pointTransactions.find(
    (transaction) => transaction.type === "INVITE_REWARD" && transaction.sourceRuleId === rule._id && transaction.changeAmount !== 0
  );
  return historicalTransaction ? Math.abs(historicalTransaction.changeAmount) : resolveInvitePointsReward(rule);
}
async function reconcileInviteRewardsForInviter(repository, inviterMemberId, rewardRules, relations, reason, actorId, now) {
  ...
  for (const rule of rewardRules.filter((item) => item.type === "INVITE_MILESTONE").sort((left, right) => left.sortOrder - right.sortOrder)) {
...
async function adjustBinding(repository, input) {
  ...
  const nextActivatedAt = nextStatus === "ACTIVATED" ? relation?.activatedAt ?? invitee.firstVisitAt ?? now : void 0;
  if (!relation) {
    relation = {
      _id: createId("invite"),
      storeId: repository.storeId,
  ...
  if (memberNeedsCleanup) {
    invitee.pendingInviteCode = void 0;
  ...
  const affectedInviterIds = Array.from(
    new Set([previousInviterMemberId, inviter._id].filter((memberId) => Boolean(memberId)))
  );
  await Promise.all(
    affectedInviterIds.map(
      (memberId) => reconcileInviteRewardsForInviter(
        repository,
        memberId,
        rewardRules,
        relations,
...
// src/handlers/admin.binding.adjust.ts
var main = defineHandler(async ({ event, repository }) => adjustBinding(repository, event));
