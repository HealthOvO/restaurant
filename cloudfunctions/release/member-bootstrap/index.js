...
// src/handlers/member.bootstrap.ts
var member_bootstrap_exports = {};
__export(member_bootstrap_exports, {
  main: () => main
});
module.exports = __toCommonJS(member_bootstrap_exports);
...
function assertInviteBindingAllowed(params) {
...
function readVerifiedPhoneFromOpenApiResult(result) {
...
async function resolveVerifiedPhoneNumber(phoneCode) {
  try {
    const result = await import_wx_server_sdk.default.openapi.phonenumber.getPhoneNumber({
      code: phoneCode
    });
...
async function bootstrapMember(repository, callerOpenId, input) {
  ...
  const now = nowIso();
  const requestedInviteCode = parsed.inviteCode ? parsed.inviteCode.trim() : void 0;
  const verifiedPhone = parsed.phoneCode ? await resolveVerifiedPhoneNumber(parsed.phoneCode) : void 0;
  ...
  if (!member) {
    ...
    member = {
      ...
      openId: callerOpenId,
      phone: verifiedPhone,
      phoneVerifiedAt: verifiedPhone ? now : void 0,
  ...
  let nextPendingInviteCode = member.pendingInviteCode ?? void 0;
  ...
  if ((member.pendingInviteCode ?? void 0) !== nextPendingInviteCode) {
...
// src/handlers/member.bootstrap.ts
var main = defineHandler(async ({ event, context, repository }) => {
