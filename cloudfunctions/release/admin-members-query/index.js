...
// src/handlers/admin.members.query.ts
var admin_members_query_exports = {};
__export(admin_members_query_exports, {
  main: () => main
});
module.exports = __toCommonJS(admin_members_query_exports);
...
function isVoucherExpired(voucher, now) {
  return voucher.status === "READY" && voucher.expiresAt <= now;
}
async function syncExpiredVoucherStatuses(repository, vouchers, now) {
  const expiredVouchers = vouchers.filter((voucher) => isVoucherExpired(voucher, now)).map((voucher) => ({
    ...voucher,
    status: "EXPIRED",
    updatedAt: now
  }));
...
function buildPaginationMeta(total, requestedPage, pageSize) {
  if (total === 0) {
    return {
      page: 1,
      pageSize,
...
async function queryMembers(repository, input) {
  ...
  const pageMembers = pagedMembers ? members : pagination.total === 0 ? [] : members.slice((pagination.page - 1) * pagination.pageSize, pagination.page * pagination.pageSize);
  ...
  const [relations, visits, rawVouchers, pointTransactions] = await Promise.all([
    ...
    typeof repository.listPointTransactionsByMemberIds === "function" ? repository.listPointTransactionsByMemberIds(memberIds) : Promise.resolve([])
  ]);
  ...
  const visitsByMemberId = visits.reduce((groups, visit) => {
    (groups[visit.memberId] ??= []).push(visit);
    return groups;
  }, {});
  const vouchersByMemberId = vouchers.reduce((groups, voucher) => {
    (groups[voucher.memberId] ??= []).push(voucher);
    return groups;
  }, {});
  const pointTransactionsByMemberId = pointTransactions.reduce((groups, transaction) => {
  ...
  const rows = pageMembers.map((member) => ({
    ...
    pointTransactions: (pointTransactionsByMemberId[member._id] ?? []).sort((left, right) => right.createdAt.localeCompare(left.createdAt)).slice(0, 20)
  }));
  return {
    ok: true,
    rows,
...
// src/handlers/admin.members.query.ts
var main = defineHandler(async ({ event, repository }) => queryMembers(repository, event));
