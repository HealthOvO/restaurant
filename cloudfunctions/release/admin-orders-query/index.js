...
// src/handlers/admin.orders.query.ts
var admin_orders_query_exports = {};
__export(admin_orders_query_exports, {
  main: () => main
});
module.exports = __toCommonJS(admin_orders_query_exports);
...
async function queryAdminOrders(repository, input) {
  ...
  const pageOrders = pagedOrders ? orders : pagination.total === 0 ? [] : orders.slice((pagination.page - 1) * pagination.pageSize, pagination.page * pagination.pageSize);
  return {
    ok: true,
    rows: pageOrders,
    pagination
...
// src/handlers/admin.orders.query.ts
var main = defineHandler(async ({ event, repository }) => queryAdminOrders(repository, event));
