...
// src/handlers/admin.opsTasks.retry.ts
var admin_opsTasks_retry_exports = {};
__export(admin_opsTasks_retry_exports, {
  main: () => main
});
module.exports = __toCommonJS(admin_opsTasks_retry_exports);
...
async function writeAudit2(repository, payload) {
  const now = nowIso3();
  await repository.addAuditLog({
    _id: createId("audit"),
    storeId: repository.storeId,
...
async function retryOpsTask(repository, input) {
  ...
  if (order.visitRecordId) {
    ...
    task.updatedAt = now;
    task.lastErrorCode = void 0;
    await repository.saveOpsTask(task);
    await writeAudit2(repository, {
      actorId: staff._id,
      actorType: "OWNER",
...
// src/handlers/admin.opsTasks.retry.ts
var main = defineHandler(async ({ event, repository }) => retryOpsTask(repository, event));
