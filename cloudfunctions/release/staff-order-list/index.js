...
// src/handlers/staff.order.list.ts
var staff_order_list_exports = {};
__export(staff_order_list_exports, {
  main: () => main
});
module.exports = __toCommonJS(staff_order_list_exports);
...
async function listStaffOrders(repository, input) {
  ...
  const orders = shouldUseRepositoryPaging ? (await repository.listOrdersPage(1, parsed.limit, parsed.status)).rows : (await repository.searchOrders(parsed.keyword, parsed.status)).slice(0, parsed.limit);
  return {
    ok: true,
    orders
  };
...
// src/handlers/staff.order.list.ts
var main = defineHandler(async ({ event, repository }) => listStaffOrders(repository, event));
