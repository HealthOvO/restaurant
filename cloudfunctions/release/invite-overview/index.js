...
// src/handlers/invite.overview.ts
var invite_overview_exports = {};
__export(invite_overview_exports, {
  main: () => main
});
module.exports = __toCommonJS(invite_overview_exports);
...
function buildInviteOverview(params) {
  ...
  const milestoneRules = rules.filter((item) => item.type === "INVITE_MILESTONE" && item.isEnabled).sort((a, b) => a.sortOrder - b.sortOrder);
  ...
  return {
    ...
    milestones: milestoneRules.map((rule) => {
      ...
      const nextRewardThreshold = rewardMode === "REPEATABLE" && threshold > 0 ? (Math.floor(activatedCount / threshold) + 1) * threshold : threshold;
      return {
        ruleId: rule._id,
        title: rule.name,
        threshold,
...
function requireMiniProgramOpenId(openId) {
  if (!openId) {
    throw new DomainError("MINI_PROGRAM_IDENTITY_REQUIRED", "\u5F53\u524D\u8BF7\u6C42\u7F3A\u5C11\u5FAE\u4FE1\u8EAB\u4EFD");
  }
  return openId;
}
async function requireMemberByOpenId(repository, openId) {
...
async function inviteOverview(repository, memberId) {
...
// src/handlers/invite.overview.ts
var main = defineHandler(async ({ context, repository }) => {
